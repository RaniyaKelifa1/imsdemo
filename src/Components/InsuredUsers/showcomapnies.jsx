...
const ShowCompany= () => {
  ...
  useEffect(() => {
    const fetchCompany = async () => {
      let Company = [];
      try {
        const response = await axios.get('http://localhost:4000/imlserver/clients', {
          params: { ClientType: 'Company' }
        });

        for (let index = 0; index < response.data.length; index++) {
          if (response.data[index].ClientType === 'Company') {
            Company.push(response.data[index]);
          }
        }
        setCompany(Company);
      } catch (err) {
  ...
  const handleDelete = async (clientId) => {
    try {
      ...
      await axios.delete(`http://localhost:4000/imlserver/clients/${clientId}`);
  ...
  const exportToExcel = () => {
    ...
    const filteredCompany = Company.filter(Company =>
      selectedRows.includes(Company.ClientID)
    );
    const ws = XLSX.utils.json_to_sheet(filteredCompany);
    XLSX.utils.book_append_sheet(wb, ws, 'Selected Company');
    const wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    const blob = new Blob([wbout], { type: 'application/octet-stream' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'selected_Company.xlsx';
    a.click();
  ...
  const filteredCompany = Company.filter(Company => {
    const normalizedPhone = normalizePhone(Company.PhoneNumber);
    ...
    <section className="min-h-screen p-4">
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold text-gray-100">Company Clients</h2>
        ...
        <table className="table-auto w-full border-collapse">
          <thead>
            <tr>
              ...
              <th className="px-4 py-2 border-b bg-gray-100 text-left text-sm font-medium text-gray-900">Company Name</th>
              <th className="px-4 py-2 border-b bg-gray-100 text-left text-sm font-medium text-gray-900">Email</th>
              <th className="px-4 py-2 border-b bg-gray-100 text-left text-sm font-medium text-gray-900">Contact Person</th>
            ...
            {filteredCompany.length > 0 ? (
              filteredCompany.map(Company => (
                <tr
                  ...
                  className={`hover:bg-sky-700 cursor-pointer ${selectedRows.includes(Company.ClientID) ? 'bg-sky-800' : ''}`}
                  onClick={() => handleRowClick(Company.ClientID)}
                >
                  <td className="px-4 py-2 text-left border">
                    <input
                      type="checkbox"
                      checked={selectedRows.includes(Company.ClientID)}
                      onChange={() => handleRowSelect(Company.ClientID)}
                    />
                  </td>
                  <td className="px-4 py-2 text-left border">{Company.Name}</td>
                  <td className="px-4 py-2 text-left border">{Company.Email}</td>
                  <td className="px-4 py-2 text-left border">{Company.ContactPerson}</td>
                  <td className="px-4 py-2 text-left border">{Company.PhoneNumber}</td>
                  <td className="px-4 py-2 text-left border">{Company.Address_City}</td>
                  <td className="px-4 py-2 text-left border">{Company.Address_Subcity}</td>
                  <td className="px-4 py-2 text-left border">{Company.Address_HouseNo}</td>
                  <td className="px-4 py-2 text-left border">{Company.Address_Wereda}</td>
                  <td className="px-4 py-2 text-left border">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        // Add navigation or edit logic here
                        console.log(`Edit Company ${Company.ClientID}`);
                      }}
              ...
              <tr>
                <td colSpan="9" className="px-4 py-2 text-center border">No Company found</td>
              </tr>
            )}
          </tbody>
        </table>
