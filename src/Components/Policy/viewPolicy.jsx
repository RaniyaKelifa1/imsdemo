...
const ShowPolicies = () => {
    ...
    const fetchData = async () => {
      try {
        const [policiesData, clientsData, providersData, vehiclesData] = await Promise.all([
          axios.get('http://localhost:4000/imlserver/policies'),
          axios.get('http://localhost:4000/imlserver/clients'),
          axios.get('http://localhost:4000/imlserver/insurance-providers'),
          axios.get('http://localhost:4000/imlserver/vehicles'),
        ]);
  ...
  const handleDelete = async (policyId) => {
    if (window.confirm('Are you sure you want to delete this policy?')) {
      try {
        await axios.delete(`http://localhost:4000/imlserver/policies/${policyId}`);
  ...
  const handleVehicleDelete = async (vehicleId) => {
    if (window.confirm('Are you sure you want to delete this vehicle?')) {
      try {
        await axios.delete(`http://localhost:4000/imlserver/vehicles/${vehicleId}`);
